...
print("gradeValue: ", gradeValue(letterGrade))
...
def gradePoint(record):
  ...
  # A4:
  gradept = float(Decimal(str(gradeValue(record['grade']))) * Decimal(str(record['credits'])))

  return gradept

# Try to run, see if it works as expected
print("gradePoint: ", gradePoint(gradeRecord))
...
def printGradeRecord(record):
  ...
  # A5:
  print("%d %s - %s (%d credits) %s  Grade points: % .1f" % (record['year'], record['semester'], record['class'], record['credits'], record['grade'], gradePoint(record))) 
...
# A6: It is a list of dictionaries
print(type(my_grades))
