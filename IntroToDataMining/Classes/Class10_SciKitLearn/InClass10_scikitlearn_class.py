...
knn_split = KNeighborsClassifier(n_neighbors=20) # instantiate with n value given
knn_split.fit(x1_train,y1_train)
y1test_pred = knn_split.predict(x1_test)
print(y1test_pred)
print(knn_split.score(x1_test,y1_test))

knn_cv = KNeighborsClassifier(n_neighbors=20) # instantiate with n value given
cv_results1 = cross_val_score(knn_cv, x1, y1, cv=10)
print("cv_result1", cv_results1)
print(np.mean(cv_results1))

xs1 = pd.DataFrame(scale(x1), columns = x1.columns)
ys1 = y1.copy()
knn_scv = KNeighborsClassifier(n_neighbors=20) # instantiate with n value given
scv_results1 = cross_val_score(knn_scv, xs1, ys1, cv=10)
print("scv_result1", scv_results1) 
print(np.mean(scv_results1))

# (2) Use the wine dataset to classify the target class ['class_0', 'class_1', 'class_2']

wx1_train, wx1_test, wy1_train, wy1_test = train_test_split(x1, y1)
knn_split = KNeighborsClassifier(n_neighbors=20) # instantiate with n value given
knn_split.fit(wx1_train,wy1_train)
wy1test_pred = knn_split.predict(wx1_test)
print(wy1test_pred)
print(knn_split.score(wx1_test,wy1_test))

knn_cv = KNeighborsClassifier(n_neighbors=20) # instantiate with n value given
wcv_results = cross_val_score(knn_cv, wine.data, wine.target, cv=10)
print("cv_result", wcv_results)
print(np.mean(wcv_results))

wxs1 = pd.DataFrame( scale(wine.data), )
wys1 = wine.target.copy()
knn_scv = KNeighborsClassifier(n_neighbors=20) # instantiate with n value given
wscv_results = cross_val_score(knn_scv, wxs1, wys1, cv=10)
print("wcsv_results", wscv_results) 
print(np.mean(wscv_results))
