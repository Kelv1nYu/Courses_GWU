...
nfl2 = pd.read_csv("nfl2008_fga.csv", index_col=[0,1] ) 
nfl2.head()
...
# 
fileDiet = os.path.join(os.getcwd(), 'Dietdata.csv')
diet = pd.read_csv(fileDiet)
diet.head()
...
#
diet['bmi'] = diet.apply(lambda x: x['pre.weight'] / ((x['Height'] / 100) ** 2), axis = 1)
diet.head()

#%%
# Task 4: create a new column to calcuate the weight loss (negative value for loss) for each person after 6 weeks
#
diet['weight_loss'] = diet.apply(lambda x: x['weight6weeks'] - x['pre.weight'], axis = 1)
diet.head()
...
#
diet['weight_loss_percentage'] = diet.apply(lambda x: x['weight_loss'] / x['pre.weight'] * 100, axis = 1)
diet.head()
...
diet.plot(x = "pre.weight", y = "Height")

# weight and height
diet.plot.scatter(x = "pre.weight", y = "Height")
# weight and loss
diet.plot.scatter(x = "pre.weight", y = "weight_loss", c = "c")
# weight and loss percentage
diet.plot.scatter(x = "pre.weight", y = "weight_loss_percentage", c = "b")
# age and weight loss
diet.plot.scatter(x = "Age", y = "weight_loss", c = "y")
# age and weight loss percentage
diet.plot.scatter(x = "Age", y = "weight_loss_percentage", c = "m")
# bmi and loss
diet.plot.scatter(x = "bmi", y = "weight_loss", c = "g")
# bmi and loss percentage
diet.plot.scatter(x = "bmi", y = "weight_loss_percentage", c = "r")
